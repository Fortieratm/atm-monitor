...
def clean_city(city):
    ...
    if not city:
        return ""
    city = city.strip()
    # Retire les codes postaux qui se glissent parfois dans la ville
    city = re.sub(r'\b[A-Z]\d[A-Z]\s*\d[A-Z]\d\b', '', city).strip()
    # Retire les numéros seuls
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if header_row:
                ...
                # Index des colonnes — basé sur les logs: id=0 name=1 addr=2 city=3 amount=8
                id_idx     = next((i for i,h in enumerate(headers_lower) if "terminal id" in h), 0)
                name_idx   = next((i for i,h in enumerate(headers_lower) if "location name" in h), None)
                if name_idx is None:
                    name_idx = next((i for i,h in enumerate(headers_lower) if "location" in h), 1)
                addr_idx   = next((i for i,h in enumerate(headers_lower) if h == "address" or "address" in h), 2)
                city_idx   = next((i for i,h in enumerate(headers_lower) if h == "city" or ("city" in h and "postal" not in h)), 3)
                postal_idx = next((i for i,h in enumerate(headers_lower) if "postal" in h), None)
                # Amount = "total cassette value"
                amount_idx = next((i for i,h in enumerate(headers_lower) if "total cassette value" in h), None)
                if amount_idx is None:
                    amount_idx = next((i for i,h in enumerate(headers_lower) if "total cassette" in h or ("cassette" in h and "value" in h)), None)

                # Fallback: cherche par $ dans la première ligne
                ...
                for cells in data_rows:
                    ...
                    city        = clean_city(cells[city_idx]) if city_idx is not None and city_idx < len(cells) else ""
                    postal      = cells[postal_idx].strip() if postal_idx is not None and postal_idx < len(cells) else ""

                    # Vérifie que la ville est bien au Québec (pas Ottawa, pas Ontario)
                    # Si la ville semble hors Québec, on la vide pour éviter de mauvaises adresses Maps
                    villes_qc = [
                        "sherbrooke","sutton","magog","granby","windsor","waterville","coaticook",
                        "cowansville","lac-megantic","ascot corner","east angus","bromont","knowlton",
                        "bedford","farnham","richmond","brome","lac-brome","montreal","québec",
                        "saint","ste","st-","st ","sainte","notre-dame","notre dame","val-","val ",
                        "saint-","la ","les ","le ","l'","rougemont","roxton","acton","stanbridge",
                        "dunham","sweetsburg","brigham","mystic","pike river","abbotsford","napierville",
                        "lacolle","noyan","henryville","saint-jean","st-jean","iberville","chambly",
                        "longueuil","boucherville","varennes","vercheres","contrecoeur","sorel",
                        "tracy","saint-hyacinthe","st-hyacinthe","saint-lambert","greenfield park",
                        "saint-constant","delson","candiac","la prairie","brossard","saint-hubert",
                        "lacadie","saint-blaise","hemmingford","huntingdon","ormstown","chateauguay",
                        "chateauguay","mercier","sainte-martine","valleyfield","salaberry","beauharnois",
                        "saint-timothee","melocheville","saint-louis","saint-stanislas","sainte-cecile",
                        "waterloo","warden","roxton pond","eastman","sainte-anne","sutton","cowansville",
                        "lac brome","foster","iron hill","west brome","glen sutton","abercorn",
                        "mansonville","potton","bolton","georgeville","north hatley","hatley",
                        "lennoxville","fleurimont","rock forest","deauville","bromptonville",
                        "windsor mills","richmond","melbourne","kingsey falls","drummondville",
                        "victoriaville","thetford mines","black lake","saint-joseph","beauceville",
                        "saint-georges","courcelles","saint-martin","saint-honoré","saint-ludger",
                        "lac-etchemin","sainte-justine","saint-prosper","saint-zacharie",
                        "triton","tricot","tricoville","3098"
                    ]

                    city_lower = city.lower()
                    city_is_qc = any(v in city_lower for v in villes_qc) if city else False

                    # Si la ville n'est pas reconnue comme QC, on garde quand même mais on force la province
                    province = "QC"
            ...
            if terminals:
                print(f"MT sample[0]: name={terminals[0]['name']} addr={terminals[0]['address']} city={terminals[0]['city']}")
